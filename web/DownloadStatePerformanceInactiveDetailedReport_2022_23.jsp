...
<%
    ...
    try {
        ...
        while (resultSet.next()) {
            filename = "SA-" + v_sa_no + " Inactive Camps Report 2022-23 " + resultSet.getString("downloaded_on") + ".xls";
        }
        resultSet.close();
        stmt.close();
        conn.close();
                ...
                <div class="table-responsive" style="vertical-align: middle; height: 480px; background-color: #fff;">
                    <table id="summary" class="table table-bordered table-responsive table-hover" border="1">
                        <thead style="background-color: #01354a; color: #ffe114;">   
                            <tr>
                                <th colspan="100" style="background-color: yellow; border-color: #fff; color: #01354a; text-align: left;"><h3><B>SA <%=v_sa_no%> - PERFORMANCE REPORT 2022-23 - INACTIVE CAMPS</B></h3></th>
                            </tr>  
                            <tr>
                        ...
                        <%
                            ...
                            try {
                                ...
                                sql = "select (select status from camps_master where upper(camp)=upper(a.camp)) camp_status,(nvl(ffr_actual,'0')+nvl(ffo_actual,'0')) ffr_ffo,"
                                        ...
                                        + "a.* from STATE_Detailed_Report_22_23fa a where sa_no='" + v_sa_no + "' and upper(Camp) in (select upper(camp) from camps_master where status='In-Active') and decode(a.region,'SA_Total','94','Inactive Camps','95','PWT SF','96','SA_Suspense','97','Total','98',a.region)>0 order by sa_no,a.region,area,camp";
