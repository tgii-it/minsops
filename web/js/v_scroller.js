                ...
                $.ajax({
                    ...
                    success: function (xml) {
                        ...
                        $(xml).find('news').each(function () {
                            ...
                            description.append($('<h1/>').html("<a href='" + url + "' style='color: navy; font-weight: bold; text-decoration: underline; font-size: 12px;'>" + htext + "</a>"));
                            
                            var newsText = $(this).find('camp').text();
                            if (newsText.length > 80) {
                                newsText = newsText.substr(0, 80) + "...";
                            }
                            description.append($('<div/>').addClass('camp').html(newsText));
                            
                            var newsText = $(this).find('goal').text();
                            if (newsText.length > 80) {
                                newsText = newsText.substr(0, 80) + "...";
                            }
                            description.append($('<div/>').addClass('goal').html(newsText));
                            
                            var newsText = $(this).find('venue').text();
                            if (newsText.length > 80) {
                                newsText = newsText.substr(0, 80) + "...";
                            }
                            description.append($('<div/>').addClass('venue').html(newsText));
                            
                            var newsText = $(this).find('speaker').text();
                            if (newsText.length > 80) {
                                newsText = newsText.substr(0, 80) + "...";
                            }
                            description.append($('<div/>').addClass('speaker').html(newsText));
