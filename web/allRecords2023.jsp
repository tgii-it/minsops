...
<html>
    <head>
        ...
        <title>NC 2023 | All Records</title>
                        ...
                        <div class='panel panel-default panel-accent-gold'>
                            <div class="panel-heading" style="background-color: #fff;">
                                <h3 class="panel-title" style="vertical-align: middle; font-size: 14px; font-weight: bold; color: #14599f; letter-spacing: 2px;">
                                    <i class="fa fa-fw fa-share"></i>&nbsp; NC 2023 : All Records
                                </h3>
                            </div>
                                    ...
                                    <tbody id='mytab' class="table-search"> 
                                        <%  String gid = "";
                                            ...
                                            try {
                                                ...
                                                String sql = "select (decode(lwife,'Yes','Both Gideon '||chr(38)||' Auxiliary','No','Gideon Alone','AAA','Auxiliary Alone',lwife)) g_a_b,(select state_asso from camps_master where camp = a.camp) state_association,(select region from camps_master where camp = a.camp) region,(select area from camps_master where camp = a.camp) area,a.camp,a.status,rid,gmid,amid,rwifename,nvl(gidmobile,'') gmobile,nvl(auxmobile,'') amobile,nvl(gidemail,'NA') gemail,nvl(auxemail,'NA') aemail,modified_on submitted_date,total_count,total_amount,dfood,wamount,cplanguage,churchaff,darrival,paymodeselect,case when paymodeselect='PG' then (select to_char(urefno) from pg_response where substr(refno,6,20)=a.rid and status='Success') else nb_utr_no end tr_refno,case when paymodeselect='PG' then (select status from pg_response where substr(refno,6,20)=a.rid and status='Success') else '-' end tr_status,case when paymodeselect='PG' then (select tr_date_time from pg_response where substr(refno,6,20)=a.rid and status='Success') else to_char(d1nb) end tr_date_time,case when paymodeselect='PG' then (select paymodeselect from pg_response where substr(refno,6,20)=a.rid and status='Success') else 'Offline' end tr_paymodeselect,case when paymodeselect='PG' then (select tr_amount from pg_response where substr(refno,6,20)=a.rid and status='Success') else total_amount end tr_amount,a.* from NC2023 a order by a.rid";
                                                System.out.println("SQL " + sql);
                                                ResultSet resultSet = statement.executeQuery(sql);
                                                while (resultSet.next()) {
                                                    g_a_b = resultSet.getString("g_a_b");
                                                    rid = resultSet.getString("rid");
                                                    gid = resultSet.getString("gmid");
                                                    aid = resultSet.getString("amid");
                                                    camp = resultSet.getString("camp");
                                                    gname = resultSet.getString("gideonname");
                                                    wname = resultSet.getString("rwifename");
...
//                                                    gamount = resultSet.getString("gidamount");
                                                    ...
                                                    food = resultSet.getString("dfood");
                                                    ...
                                                    tr_paymode = resultSet.getString("tr_paymodeselect");
                                                    tr_amount = resultSet.getString("tr_amount");
                                                    remarks = resultSet.getString("trremarks");
                                                    paymethod = resultSet.getString("paymodeselect");
                                                    if (gname == null) {
                                                        gname = "";
                                                    }
                                                    if (wname == null) {
                                                    ...
                                                    if (amobile == "0" || amobile == null || amobile == "null") {
                                                        amobile = "";
                                                    }
