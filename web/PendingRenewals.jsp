...
<html>
    <head>
        ...
        <title>TGII | Pending Membership Renewals MIS</title>
                            ...
                            <div class='panel panel-default panel-accent-gold'>
                                <div class="panel-heading" style="background-color: #fff;">
                                    <h3 class="panel-title" style="vertical-align: middle; font-size: 14px; font-weight: bold; color: #14599f; letter-spacing: 2px;">
                                        <i class="fa fa-fw fa-share"></i>&nbsp; Regular Renewals
                                        ...
                                        <tbody id='mytab' class="table-search"> 
                                            <%  String mid = "";
                                                ...
                                                try {
                                                    ...
                                                    // String sql = "select Cars_get_StateAsso(camp) state_association,mid,nvl(camp,'-') camp,nvl(address,'-')||'<br>'||nvl(address2,'-')||'<br>'||nvl(street,'-')||'<br>'||nvl(city,'-')||'<br>'||nvl(state,'0')||' '||nvl(pincode,'0') address,nvl(orbitid,'0') orbitid,nvl(name,'-') name,nvl(phone,'-') phone,nvl(mobile2,'-')mobile2,nvl(mobile3,'-')mobile3,nvl(email,'-')email,nvl(mtype,'-')mtype,nvl(status,'-')status,to_char(annual_end_date,'DD-MON-YY') annual_end_date,to_char(annual_start_date,'DD-MON-YY') annual_start_date, Get_SanProcess_Mtype(mid) San_Mem_Type,nvl(e503_Get_remitter(mid),'NA') remitter from membership_weekly_raw where upper(mid) in (select upper(mid) from san_process1 where record_status='Pending' and renewal_contactperson in (select rid from e503) and mid is not null) and upper(mtype) like '%ANNUAL%' and annual_end_date=to_date('31-10-19','DD-MM-YY') and upper(camp) not in (select upper(camp) from camps_master where sa_no in (12,13,14)) order by annual_end_date";
                                                    String sql = "select Cars_get_StateAsso(camp) state_association,mid,nvl(camp,'-') camp,nvl(address,'-')||'<br>'||nvl(address2,'-')||'<br>'||nvl(street,'-')||'<br>'||nvl(city,'-')||'<br>'||nvl(state,'0')||' '||nvl(pincode,'0') address,nvl(orbitid,'0') orbitid,nvl(name,'-') name,nvl(phone,'-') phone,nvl(mobile2,'-')mobile2,nvl(mobile3,'-')mobile3,nvl(email,'-')email,nvl(mtype,'-')mtype,nvl(status,'-')status,to_char(annual_end_date,'DD-MON-YY') annual_end_date,to_char(annual_start_date,'DD-MON-YY') annual_start_date, Get_SanProcess_Mtype(mid) San_Mem_Type,nvl(e503_Get_remitter(mid),'NA') remitter from membership_weekly_raw where upper(mid) in (select upper(mid) from san_process1 where record_status='Pending' and renewal_contactperson IN (SELECT MID from  e503_payment where e503date > to_date('23/11/2020','DD/MM/YYYY')) and mid is not null) and upper(mtype) like '%ANNUAL%' and annual_end_date<=to_date('31-10-20','DD-MM-YY') and mid not in (select mid from v_renewal_reinstated) order by annual_end_date";
                                        ...
                                        <div class="form-group col-md-2">
                                            ...
                                            <div class='input-group'>
                                                ...
                                                <select name="renewalStatus" id="renewalStatus" class="form-control" required >
                                                    ...
                                                    <option value="Renewed_PTP_Confirmed">- PTP Confirmed -</option>
                                                    <option value="Renewed_WTR_Confirmed">- WTR Confirmed -</option>
